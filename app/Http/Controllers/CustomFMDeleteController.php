...
class CustomFMDeleteController extends \UniSharp\LaravelFilemanager\Controllers\LfmController
{
    /**
    ...
    public function getDelete()
    {
        ...
        foreach ($item_names as $name_to_delete) {
            ...
            if (is_null($name_to_delete)) {
                array_push($errors, parent::getErrors('folder-name'));
                continue;
            }

            if (! $this->lfm->setName($name_to_delete)->exists()) {
                array_push($errors, parent::getErrors('folder-not-found', ['folder' => $file_path]));
                continue;
            }

            if ($this->lfm->setName($name_to_delete)->isDirectory()) {
                // if (! $this->lfm->setName($name_to_delete)->directoryIsEmpty()) {
                //     array_push($errors, parent::getErrors('delete-folder'));
                //     continue;
                // }
