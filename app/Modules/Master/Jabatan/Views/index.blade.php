...
@section('content')
    ...
    <div class="col-md-12 text-danger alert alert-danger">
        Kode Jabatan tidak dapat diubah. Apabila ada penyesuaian kode maka diperlukan tambah data dan pemetaan yang lainnya.
    </div>
    <div class="row">
        <div class="col">
    ...
    <script type="text/javascript">
        var oTable = $('#main-table').myDataTable({
            ...
            customRow: function(row, data) {
                ...
                if (data.status == '1') {
                    $('td:eq(2)', row).html(
                        '<span class="badge bg-success-subtle text-success text-uppercase">' + __('Aktif') +
                        '</span>');
                } else {
                    if (data.deleted_at != null) {
                        $('td:eq(2)', row).html(
                            '<span class="badge bg-danger-subtle text-danger text-uppercase">' + __(
                                'Deleted') + '</span>');
