(function ($) {
    ...
    $.fn.myDataTable = function (options) {
        ...
        var defaults = {
            ...
            filter: false,
            sLengthSelect: 'select2 form-select',
            searching: false,
            lengthChange: true,
            dom: "<'row'<'#" + object.attr('id') + "_length.col-sm-6'l><'#" + object.attr('id') + "_filter.col-sm-6 text-end'f>>" +
                "<'row'<'col-sm-12'tr>>" +
            ...
            language: {
                sLengthMenu: "_MENU_",
                sZeroRecords: '<div class="text-center"><i class="bx bx-search-alt-2 bx-sm bx-tada"></i><div class="mt-2">Maaf! Tidak ada hasil yang ditemukan</div></div>',
        ...
        var renderButtons = function () {
            ...
            $.each(options.buttons, function (idx, val) {
                ...
                if (typeof val.id != 'undefined' && val.id == 'add') {
                    def = {
                        icon: '<i class="bx bx-plus-circle bx-xs align-middle"></i>',
                        title: __("Tambah"),
                ...
                if (typeof attr.modal != 'undefined' && attr.modal) {
                    b.attr('data-bs-toggle', attr.toggle);
                    b.attr('data-bs-target', attr.modal);
                }
            ...
            if (totalButton == 0) {
                ...
                // boxButton.html('<div class="btn-group" role="group" aria-label="button">'+ boxButton +'</div>');
            }
        };
        ...
        var dataTable = object.DataTable({
            ...
            sLengthSelect: options.sLengthSelect,
            ...
            rowCallback: function (row, data, displayNum) {
                ...
                if (typeof data.checkbox != 'undefined') {
                    ...
                    var box = $('<div \>');
                    box.addClass('form-check form-check-inline mb-1');
                    box.append('<input type="checkbox" class="custom-control-input dt-checkbox form-check-input" id="cb-' + displayNum + '" name="_id[]" ' + disabled + ' value="' + data.id + '">');
                    box.append('<label class="custom-control-label" for="cb-' + displayNum + '"></label>');
                    $('td:first', row).html(box);
                }

                // set action
                var act = ['edit', 'delete', 'restore'];
                ...
                if (options.actions.length > 0) {
                    ...
                    $.each(options.actions, function (idx, val) {
                        ...
                        if (typeof val.id != 'undefined') {
                            if (val.id == 'edit') {
                                def = {
                                    icon: '<i class="bx bx-edit bx-xs align-middle"></i>',
                                    title: __("Edit"),
                                ...
                                def = {
                                    icon: '<i class="bx bx-trash bx-xs align-middle"></i>',
                                    title: __("Delete"),
                                    className: 'btn btn-danger btn-delete p-1'
                                };
                            } else if (val.id == 'restore') {
                                def = {
                                    icon: '<i class="bx bx-refresh bx-xs align-middle"></i>',
                                    title: __("Restore"),
                                    className: 'btn btn-outline-success btn-restore p-1'
                                };
                            }
                        } else {
                            def = {
