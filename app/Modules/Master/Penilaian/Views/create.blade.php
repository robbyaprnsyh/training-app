...
@section('content')
    ...
    <div class="modal-body pb-2">
        ...
        <input type="hidden" name="tipe_penilaian" value="{{ $parameter->tipe_penilaian }}">

        <div class="form-group row p-0 mb-1">
            <label class="col-sm-3 col-form-label">{{ __('Parameter') }}</label>
            <div class="col-sm-9">
                {{ Form::text('parameter_name', $parameter->name ?? '-', ['class' => 'form-control', 'readonly']) }}
            </div>
        </div>

        <div class="form-group row p-0 mb-1" @if ($parameter->tipe_penilaian !== 'KUANTITATIF') hidden @endif>
        ...
        <div class="form-group row p-0 mb-1" @if ($parameter->tipe_penilaian !== 'KUALITATIF') hidden @endif>
        ...
        <div class="form-group row p-0 mb-1">
            ...
            <div class="col-sm-9">
                {{ Form::textarea('analisa', null, ['class' => 'form-control', 'rows' => 2]) }}
            </div>
        </div>

        <div class="form-group row p-0 mb-1 mt-3">
            <label class="col-sm-3 col-form-label">{{ __('Status') }}</label>
            <div class="col-sm-9">
                <div class="form-check form-check-inline">
                    <input class="form-check-input" type="radio" name="status" id="status_draft" value="0" checked>
                    <label class="form-check-label" for="status_draft">{{ __('Draft') }}</label>
                </div>
                <div class="form-check form-check-inline">
                    <input class="form-check-input" type="radio" name="status" id="status_selesai" value="1">
                    <label class="form-check-label" for="status_selesai">{{ __('Selesai') }}</label>
                </div>
            </div>
        </div>
    </div>
